...
odefunc_pele = get_ode_func_gradient_problem_pele!(
    pele_wrapped_python_pot;
    jac_prototype = sparse_hess,
)
...
setups = [
    ...
    Dict(
        :alg => QNDF(
            linsolve = KrylovJL_GMRES(),
            precs = algebraicmultigrid_with_smoothing,
            concrete_jac = true,
            autodiff = false,
        ),
    ),
...
savefig(
    "comparison_cvode_cell_lists_OMP_THREADS_particles_n_" *
    string(natoms) *
    "_" *
    string(ENV["OMP_NUM_THREADS"]),
)
