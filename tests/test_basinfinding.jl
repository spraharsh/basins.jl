...
using Sundials
test_radii = [1.0339553713017888, 1.0414206741450018, 0.9823496299849702, 0.9932573064034739, 1.0293308537316554, 1.0148667925424708, 1.003247377427417, 0.9945491307459141, 1.3640052726416674, 1.510203114149589, 1.3517765010522012, 1.3466037328512679, 1.4278237686452713, 1.456814073575481, 1.375755177700484, 1.3868699122638386]
...
println(system_energy!(pot, test_coords))
ba_qndf = BasinAssigner(QNDF(autodiff=true), tol, tol, 1e-6) # The convergence tolerance is error in gradient
ba_cvode_bdf = BasinAssigner(CVODE_BDF(), tol, tol, 1e-6)
ba_auto_switch = BasinAssigner(AutoTsit5(Rosenbrock23(autodiff=true)), tol, tol, 1e-6)

@time final_cvode = find_corresponding_minimum(ba_cvode_bdf, ode_problem, test_coords, 1000, pot)
println(final_cvode)
...
@time final_auto_switch = find_corresponding_minimum(ba_auto_switch, ode_problem, test_coords, 1000, pot)
